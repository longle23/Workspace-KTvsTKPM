...
//	14. Cho biết các chuyến bay có thể ñược thực hiện bởi máy bay Airbus A320
	@Query(value = "SELECT * from chuyenbay where DoDai < ( select TamBay from maybay where Loai =N'Airbus A320')", nativeQuery = true)
...
//	đường bay nào có thể đáp ứng yêu cầu này.
	@Query(value = "select * from chuyenbay where GaDi in"+
			"(select GaDen from chuyenbay) and GaDen in (select GaDi from chuyenbay)", nativeQuery = true)
...
//	đó.
	@Query(value = "select GaDi, count(*) as SoChuyenBay from chuyenbay\r\n"
...
//	chuyến bay khởi hành từ ga đó.
	@Query(value = "select GaDi, sum(chiPhi) as Tongchiphi from chuyenbay\r\n"
			+ "group by GaDi", nativeQuery = true)
...
//	21. Với mỗi địa điểm xuất phát cho biết có bao nhiêu chuyến bay có thể khởi hành trước 12:00.
	@Query(value = "select GaDi, count(*) as soluong from chuyenbay where GioDi<'12:00:00'\r\n"
			+ "group by GaDi", nativeQuery = true)
...
//	28. Tìm các chuyến bay có thể được thực hiện bởi tất cả các loại máy bay Boeing
	@Query(value = "\r\n"
			...
			+ "having DoDai<= all(select TamBay from maybay where Loai like '%Boeing%')", nativeQuery = true)
